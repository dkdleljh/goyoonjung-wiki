#!/usr/bin/env python3
"""Promote campaign dates in pages/pictorials/campaign.md when objectively extractable.

Safe scope:
- Only touches blocks where `- 날짜: (확인 필요)`.
- If the block contains a YouTube link, fetch the watch page and extract an objective
  publish/upload date (YYYY-MM-DD).
- Write the date back into the same block.

Non-goals:
- No guessing from secondary sources.
- No Instagram parsing.

This is best-effort and skips on any parsing failure.
"""

from __future__ import annotations

import os
import re
import sys
import time

import requests

BASE = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
FILE = os.path.join(BASE, "pages", "pictorials", "campaign.md")

UA = {
    "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/120 Safari/537.36",
    "Accept-Language": "en-US,en;q=0.9,ko;q=0.8",
}
TIMEOUT = 25

YOUTUBE_URL_RE = re.compile(r"https?://(?:www\.)?(?:youtu\.be/|youtube\.com/watch\?v=)([A-Za-z0-9_-]{6,})")
DATE_PUBLISHED_RE = re.compile(r"\"datePublished\"\s*:\s*\"(20\d{2}-\d{2}-\d{2})\"")
DATE_TEXT_RE = re.compile(r"\"dateText\"\s*:\s*\{\s*\"simpleText\"\s*:\s*\"(20\d{2})\.\s*(\d{1,2})\.\s*(\d{1,2})\.?\"", re.S)


def to_watch_url(url: str) -> str:
    if "youtu.be/" in url:
        vid = url.rstrip("/").split("/")[-1]
        return f"https://www.youtube.com/watch?v={vid}"
    return url


def fetch_date_published(youtube_url: str) -> str | None:
    url = to_watch_url(youtube_url)
    try:
        r = requests.get(url, headers=UA, timeout=TIMEOUT)
        r.raise_for_status()
        html = r.text
    except Exception:
        html = ""

    if html:
        m = DATE_PUBLISHED_RE.search(html)
        if m:
            return m.group(1)
        m2 = DATE_TEXT_RE.search(html)
        if m2:
            y, mo, d = int(m2.group(1)), int(m2.group(2)), int(m2.group(3))
            return f"{y:04d}-{mo:02d}-{d:02d}"

    # Fallback: yt-dlp (objective, more robust)
    try:
        import subprocess

        out = subprocess.check_output(
            ["yt-dlp", "--no-warnings", "--skip-download", "--print", "%(upload_date)s", url],
            stderr=subprocess.DEVNULL,
            timeout=30,
        ).decode("utf-8", "ignore").strip()
        if re.fullmatch(r"20\d{2}[01]\d[0-3]\d", out):
            return f"{out[0:4]}-{out[4:6]}-{out[6:8]}"
    except Exception:
        pass

    return None


def block_range(lines: list[str], start: int) -> tuple[int, int]:
    i = start
    j = i + 1
    while j < len(lines) and not lines[j].lstrip().startswith("- 날짜:") and not lines[j].startswith("## "):
        j += 1
    return i, j


def main() -> int:
    if not os.path.exists(FILE):
        print("promote_campaign_dates: missing file")
        return 0

    lines = open(FILE, "r", encoding="utf-8").read().splitlines(True)
    changed = 0

    i = 0
    while i < len(lines):
        if lines[i].lstrip().startswith("- 날짜:"):
            s, e = block_range(lines, i)
            block = "".join(lines[s:e])

            if re.search(r"-\s*날짜:\s*\(확인 필요\)", block):
                ym = YOUTUBE_URL_RE.search(block)
                if ym:
                    yt_url = ym.group(0)
                    date = fetch_date_published(yt_url)
                    if date:
                        for k in range(s, e):
                            if lines[k].lstrip().startswith("- 날짜:"):
                                indent = re.match(r"^\s*", lines[k]).group(0)
                                lines[k] = f"{indent}- 날짜: {date} (YouTube upload_date)\n"
                                changed += 1
                                break
                time.sleep(0.4)

            i = e
        else:
            i += 1

    if changed:
        with open(FILE, "w", encoding="utf-8") as f:
            f.write("".join(lines))

    print(f"promote_campaign_dates: updated={changed}")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
